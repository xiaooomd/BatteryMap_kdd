...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'early_prediction':
            ...
            self.pred_len = configs.early_cycle_threshold # Ensure Decoder output length matches input
        ...
        if self.task_name == 'early_prediction':
             # For feature mode, we project flattened [B, Cycles * D_Model] -> [B, 1]
    ...
    def forward(self, cycle_curve_data, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None, return_embedding=False):
        '''
        params:
            cycle_curve_data: [B, L, num_variables, fixed_length_of_curve] OR [B, L, Features]
            x_mark_enc: [B, L] (used as curve_attn_mask in some contexts)
        '''
        # Alias for compatibility with existing logic
        curve_attn_mask = x_mark_enc
        ...
        if cycle_curve_data.dim() == 3:
            # [Batch, Cycles, Features] -> already flattened
            cycle_curve_data = cycle_curve_data
        else:
            # [Batch, Cycles, Vars, Len] -> Flatten
