...
class Model(nn.Module):
    ...
    def __init__(self, configs, conv_kernel=[12, 16]):
        ...
        # kernel of decomposition operation
        decomp_kernel = []
        # kernel of isometric convolution
        isometric_kernel = []
        ...
        # Define act and dropout for classification method (used regardless of task_name)
        self.act = F.gelu
        ...
        if self.task_name == 'imputation':
            ...
            # Projection input: [B, seq_len * enc_in] after reshape
            # After conv_trans (c_out channels) + trend (enc_in channels) -> [B, L, enc_in]
            # After reshape: [B, L * enc_in]
            if self.feature_type == 'extracted_features':
                projection_input_dim = configs.enc_in * configs.early_cycle_threshold
            else:
                projection_input_dim = configs.enc_in * configs.early_cycle_threshold * configs.charge_discharge_length
            self.projection = nn.Linear(projection_input_dim, configs.output_num)
        elif self.task_name == 'early_prediction':
            # Early prediction: scalar regression output
            # Projection input: [B, seq_len * enc_in] after reshape
            if self.feature_type == 'extracted_features':
                projection_input_dim = configs.enc_in * configs.early_cycle_threshold
            else:
                projection_input_dim = configs.enc_in * configs.early_cycle_threshold * configs.charge_discharge_length
            self.projection = nn.Linear(projection_input_dim, 1)
        else:
            # Default projection for undefined task types
            self.projection = nn.Linear(configs.d_model, configs.c_out, bias=True)
    ...
    def forward(self, cycle_curve_data, curve_attn_mask, return_embedding=False):
        '''
        params:
            cycle_curve_data: [B, L, num_variables, fixed_length_of_curve] for PKL curve mode
