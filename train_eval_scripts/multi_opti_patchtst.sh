#!/bin/bash
# PatchTST multi-dataset parallel hyperparameter optimization script (Recommended)
# Purpose: Parallel optimization on all 17 datasets, obtaining independent optimal parameters for each dataset

echo "========================================"
echo "PatchTST Multi-dataset Parallel Hyperparameter Optimization"
...
python run.py multi-dataset-opt \
    ...
    --output_dir ./hyperparam_results/multi_dataset/PatchTST

echo ""
echo "All optimizations completed! Results summarized in: ./hyperparam_results/multi_dataset/PatchTST"
