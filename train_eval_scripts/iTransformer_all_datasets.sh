#!/bin/bash
# Batch train iTransformer on all small datasets
...
# iTransformer training parameters
...
do
    ...
    checkpoints=./checkpoints/iTransformer_${dataset}_$(date +%Y%m%d_%H%M%S)
    ...
    CUDA_VISIBLE_DEVICES=2,3 accelerate launch --multi_gpu --num_processes $num_process --num_machines 1 --mixed_precision no --dynamo_backend no --main_process_port $master_port run_main.py \
      ...
      --is_training 1 \
      --root_path $root_path \
      --model_id iTransformer_${dataset} \
      --model $model_name \
      --features MS \
      --seed $seed \
      --seq_len $seq_len \
