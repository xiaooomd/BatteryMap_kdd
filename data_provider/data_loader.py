...
def my_collate_fn_withId(samples):
    ...
    if cycle_curve_data.dim() == 3:
        # Shape [Batch, Cycles, Features]
        # Mask shape [Batch, Cycles] -> expand to [Batch, Cycles, Features]
        tmp_curve_attn_mask = curve_attn_mask.unsqueeze(-1) * torch.ones_like(cycle_curve_data)
        cycle_curve_data[tmp_curve_attn_mask==0] = 0
    elif cycle_curve_data.dim() == 4:
        # Shape [Batch, Cycles, Channels, Length] (for CNN/image-like data)
        # Mask shape [Batch, Cycles] -> expand to [Batch, Cycles, Channels, Length]
        tmp_curve_attn_mask = curve_attn_mask.unsqueeze(-1).unsqueeze(-1) * torch.ones_like(cycle_curve_data)
        cycle_curve_data[tmp_curve_attn_mask==0] = 0
    else:
        # Unknown dimension, try to infer
        raise ValueError(f"Unexpected cycle_curve_data dimension: {cycle_curve_data.dim()}, shape: {cycle_curve_data.shape}")
...
class Dataset_CustomFeatures(Dataset):
    def __init__(self, args, flag='train', feature_scaler=None, label_scaler=None, **kwargs):
        self.args = args
        self.flag = flag
        self.root_path = args.root_path
        self.early_cycle_threshold = args.early_cycle_threshold
        ...
        self.data_y = []
        self.masks = []
        self.weights = [] # Placeholder if needed
        self.dataset_ids = []
        self.seen_unseen_ids = []
        self.life_classes = []
        self.scaled_life_classes = []

        # 1. File Discovery & Splitting
        if not os.path.isdir(self.root_path):
             ...
             # Attempt to use split_recorder
             dataset_name = args.dataset

             # Map flag to split_recorder attribute suffix
             suffix_map = {
                 'train': '_train_files',
                 'val': '_val_files',
                 'test': '_test_files'
             ...
             # Map dataset name to split_recorder prefix if inconsistent
             split_recorder_prefix_map = {
                 'NAion2024': 'NAion_2024',
                 'ZN-coin2024': 'ZN_2024',
                 'ZN-coin42': 'ZN_42',
                 'CALB42': 'CALB_42',
                 'CALB2024': 'CALB_2024',
                 'NAion42': 'NAion_42',
                 'li_results_ALL': 'li_results_ALL'  # Added: support for all-dataset mixed mode
             }

             sr_dataset_name = split_recorder_prefix_map.get(dataset_name, dataset_name)
             attr_name = f"{sr_dataset_name}{suffix_map.get(flag, '')}"
             target_files_pkl = getattr(split_recorder, attr_name, None)

             if target_files_pkl is not None:
                 # Filter all_files based on target_files_pkl
                 # Note: split_recorder uses .pkl filenames, we have .csv filenames on disk
                 # We need to robustly match them.

                 # Create a set of target base names (without extension or handled consistently)
                 # The robust way is to replace .pkl with .csv in the target list
                 target_files_csv = set([f.replace('.pkl', '.csv') for f in target_files_pkl])

                 # Special handling for Tongji: split_recorder has '-#' but file system might be different
                 # Logic from line 825: key_name = key_name.replace('--', '-#') implies file system has '--'
                 # So if target has '-#', we expect file system to have '--'
                 if dataset_name.startswith('Tongji'):
                      target_files_csv = set([f.replace('.pkl', '.csv').replace('-#', '--') for f in target_files_pkl])
                 elif dataset_name == 'NAion2024':
                      # NAion2024 uses 'NA-ion' prefix in pkl/json but directory is 'NA'
                      # Filenames might be consistent (NA-ion_...)
                      pass
                 elif dataset_name == 'ZN-coin2024':
                      # ZN-coin2024 uses 'ZN-coin' prefix but directory is 'ZNion'
                      pass

                 self.file_list = []
                 for f_path in all_files:
                     f_name = os.path.basename(f_path)
                     if f_name in target_files_csv:
                         self.file_list.append(f_path)

                 print(f"[{flag}] Loaded {len(self.file_list)} files using split_recorder for {dataset_name}")

             else:
                 # Fallback to random split
                 print(f"[{flag}] Warning: No split record found for {dataset_name}, using random split.")
                 all_files.sort()
                 ...
                 if flag == 'train':
                     self.file_list = all_files[:n_train]
                 elif flag == 'val':
                     self.file_list = all_files[n_train:n_train+n_val]
                 else:
                     self.file_list = all_files[n_train+n_val:]

        # 2. Data Loading & Pre-processing (Two-pass approach)
        # Pass 1: Collect valid series for Scaling (if training)
        valid_series_list = []
        labels_list = []
        ...
        for file_path in tqdm(self.file_list, desc=f"Loading {flag} data"):
            try:
                # 1. Load Label from JSON
                file_name = os.path.basename(file_path)
                prefix = file_name.split('_')[0]

                # Handle special prefixes if necessary (similar to Dataset_original)
                ...
                # Try replacing .csv with .pkl
                key_name = file_name.replace('.csv', '.pkl')

                # Special handling for Tongji filenames if needed (from Dataset_original)
                if prefix.startswith('Tongji'):
                    key_name = key_name.replace('--', '-#')

                label_dict = self.label_cache.get(prefix, {})

                if key_name in label_dict:
                    label = float(label_dict[key_name])
                else:
                    # print(f"Warning: Label not found for {key_name} in {prefix}_labels.json")
                    continue # Skip if no label found

                # 2. Load Features from CSV
                ...
                if numeric_df.isnull().any().any():
                    print(f"Warning: NaN detected in {file_path} (after slicing), filling with forward fill then 0")
                    numeric_df = numeric_df.fillna(method='ffill').fillna(0)

                features = numeric_df.values

                # Final NaN check after slicing
                if np.isnan(features).any():
                    print(f"Error: NaN still present in {file_path} after cleaning, skipping")
                    continue

                # Store only valid rows, do not pad yet
                valid_series_list.append(features)
        ...
        # 3. Normalization
        if flag == 'train':
            self.feature_scaler = StandardScaler() if feature_scaler is None else feature_scaler
            self.label_scaler = StandardScaler() if label_scaler is None else label_scaler

            # Fit on all concatenated valid data points
            if len(valid_series_list) > 0:
                all_valid_data = np.concatenate(valid_series_list, axis=0)
                self.feature_scaler.fit(all_valid_data)
                self.label_scaler.fit(np.array(labels_list).reshape(-1, 1))
        else:
            self.feature_scaler = feature_scaler
            self.label_scaler = label_scaler

        # 4. Transform & Generate Cumulative Samples
        if len(valid_series_list) > 0 and self.feature_scaler:
            transformed_series_list = [self.feature_scaler.transform(s) for s in valid_series_list]
            if self.label_scaler:
                transformed_labels = self.label_scaler.transform(np.array(labels_list).reshape(-1, 1))
            else:
                transformed_labels = np.array(labels_list).reshape(-1, 1)
        else:
            transformed_series_list = valid_series_list
            transformed_labels = np.array(labels_list).reshape(-1, 1)

        # Generate Samples
        for idx, series in enumerate(transformed_series_list):
            L, D = series.shape
            limit = min(L, self.early_cycle_threshold)
            label = transformed_labels[idx]

            # Cumulative generation: [0...i]
            # Must have at least seq_len cycles
            ...
            for i in range(start_idx, limit + 1):
                # Container [T, D]
                sample_x = np.zeros((self.early_cycle_threshold, D))
                sample_mask = np.zeros((self.early_cycle_threshold,))

                # Fill data up to i
                sample_x[:i, :] = series[:i, :]
                sample_mask[:i] = 1.0

                self.data_x.append(sample_x)
                self.masks.append(sample_mask)
                self.data_y.append(label)

                # Fill dummy metadata
                self.life_classes.append(0)
                self.scaled_life_classes.append(0)
                self.weights.append(1.0)
                self.dataset_ids.append(0)
                self.seen_unseen_ids.append(0)

    def __getitem__(self, index):
        # Must return dict compatible with collate_fn
        # Keys: 'cycle_curve_data', 'labels', 'curve_attn_mask', 'life_class', 'dataset_id', 'seen_unseen_id'

        return {
            'cycle_curve_data': torch.Tensor(self.data_x[index]),
            'labels': torch.Tensor(self.data_y[index]),
            'curve_attn_mask': torch.Tensor(self.masks[index]),
            'life_class': self.life_classes[index],
            'scaled_life_class': self.scaled_life_classes[index],
            'weight': self.weights[index],
            'dataset_id': self.dataset_ids[index],
            'seen_unseen_id': self.seen_unseen_ids[index]
        }
    ...
    def return_feature_scaler(self):
