...
parser.add_argument('--swanlab_project', type=str, default='new_LifeBaseline', help='SwanLab project name')
...
for ii in range(args.itr):
    ...
    train_data, train_loader, train_target_data, train_target_loader = data_provider_func(
        args, 'train', use_da=True, tokenizer=None, 
        sample_weighted=args.weighted_sampling, target_dataset=args.target_dataset)
    ...
    vali_data, vali_loader = data_provider_func(
        args, 'val', tokenizer=None, label_scaler=label_scaler, 
        life_class_scaler=life_class_scaler, sample_weighted=args.weighted_sampling)
    accelerator.print("Loading test samples......")
    test_data, test_loader = data_provider_func(
        args, 'test', tokenizer=None, label_scaler=label_scaler, 
        life_class_scaler=life_class_scaler, sample_weighted=args.weighted_sampling)
    ...
    swanlab_tracker = SwanLabTracker(
        ...
        run_name=args.swanlab_experiment_name or nowtime,
        config=args.__dict__,
        enabled=accelerator.is_local_main_process and not args.disable_swanlab,
    ...
    for epoch in range(args.train_epochs):
        ...
        if accelerator.is_local_main_process and swanlab_tracker.available:
            swanlab_tracker.log({"epoch": epoch, "train_loss": train_loss, "vali_RMSE": vali_rmse, "vali_MAPE": vali_mape, "vali_acc1": vali_alpha_acc1, "vali_acc2": vali_alpha_acc2,
...
if accelerator.check_trigger() and accelerator.is_local_main_process:
    if swanlab_tracker.available:
        swanlab_tracker.log({"epoch": epoch+1, "train_loss": train_loss, "vali_RMSE": best_vali_RMSE, "vali_MAPE": best_vali_MAPE, "vali_acc1": best_vali_alpha_acc1, "vali_acc2": best_vali_alpha_acc2,
