...
Example:
    Single dataset:
        python run.py hyperopt --method grid --model MLP --dataset HUST

    Multi-dataset aggregation:
        python run.py hyperopt --method pso --model MLP --datasets HUST CALB CALCE --aggregation mean
"""
...
from configs.hyperparam_search_config import get_pso_search_space, get_search_space
...
def run_single_experiment(params: Dict[str, Any],
    ...
    env['SWANLAB_MODE'] = 'disabled'  # Disable SwanLab to avoid remote logging in search trials
...
def main():
    ...
    if args.method == 'grid':
        ...
        search_space_label = 'grid/discrete'
    else:
        search_space = get_pso_search_space(args.model)
        search_space_label = 'pso'
    ...
    print(f"Search Space Type: {search_space_label}")
